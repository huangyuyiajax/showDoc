...
import com.api.showDoc.javaParser.xdoc.utils.TagUtils;
...
public class SpringWebFramework implements Framework {
    ...
    private SpringApiAction buildSpringApiAction(ApiAction apiAction, boolean isjson) {
        ...
        this.setRequiresPermissions(apiAction, saa);
        if (!isMappingMethod) {
            return null;
        }
    ...
    /**
     * 设置shiro  权限标识 RequiresPermissions
     */
    private boolean setRequiresPermissions(ApiAction apiAction, SpringApiAction saa) {
        RequiresPermissions requiresPermissions = apiAction.getMethod().getAnnotation(RequiresPermissions.class);
        if (requiresPermissions != null) {
            saa.setRequiresPermissions(this.getUris(requiresPermissions.value()));
            return true;
        }
        return false;
    }
